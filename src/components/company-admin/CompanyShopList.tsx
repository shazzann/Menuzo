    ...
    try {
      ...
      const mappedShops: ManagedShop[] = ((data as any) || []).map((shop: any) => {
        ...
        return {
          ...
          logo: shop.logo || '',
          banner: shop.banner || '',
          theme: shop.theme || { primary: '#FB8500', secondary: '#1C1E22', accent: '#FB8500' },
          qrScans: 0,
          visitors: 0
        };
      });
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((shop) => (
            <div
              key={shop.id}
              className="group bg-background text-foreground border border-border/50 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-primary hover:shadow-xl hover:shadow-primary/20"
              style={getShopThemeStyles(shop.theme)}
            >
              {/* Card Header */}
              <div className="relative h-24 bg-gradient-to-br from-primary/20 to-primary/5">
                ...
                <div className="absolute -bottom-6 left-4 z-10">
                  {shop.logo ? (
                    ...
                    <div className="w-14 h-14 rounded-xl flex items-center justify-center text-primary-foreground text-lg font-bold shadow-lg border-4 border-card bg-primary">
                      {shop.name.charAt(0)}
                    </div>
                  )}
                </div>
              ...
              <div className="pt-8 px-4 pb-4">
                ...
                <div className="flex items-center gap-1.5 pt-3 border-t border-border/50">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-medium transition-colors hover:brightness-110"
                  >
        ...
        <div className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-2xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {filtered.map((shop) => (
                  <tr key={shop.id} className="border-b border-border/30 hover:bg-muted/50 transition-colors bg-background text-foreground" style={getShopThemeStyles(shop.theme)}>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        {shop.logo ? (
                          ...
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-primary-foreground text-xs font-bold shrink-0 bg-primary">
                            {shop.name.charAt(0)}
                          </div>
                        )}
                        <div>
